...
产品负责人（PO） 确定需求（Product Backlog）。
Scrum 团队 在 Sprint 计划会议上选择要开发的任务。
Sprint 迭代（1-4 周），每天开 Standup 会议检查进度。
Sprint 结束，交付产品可用版本，复盘问题。
示例：

一个射击游戏团队每 2 周开发一个新功能，如 优化子弹同步算法。
每次 Sprint 结束都会有可交付的版本。
Kanban 详细介绍
Kanban 采用"任务流动"管理开发进度：

任务按照 "待办（To Do）→ 进行中（In Progress）→ 完成（Done）" 排列。
开发人员从 "To Do" 选取任务，自由完成，无固定迭代周期。
工作流透明可视化，防止任务积压，提高效率。
